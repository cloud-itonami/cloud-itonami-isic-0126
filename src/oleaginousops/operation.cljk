(ns oleaginousops.operation
  "OperationActor -- one oleaginous-fruit plantation operation = one
  supervised actor run. The advisor (OleaginousOpsAdvisor) is sealed into
  a single node (:advise); its proposal is ALWAYS routed through the
  Oleaginous-Fruit Plantation Operations Governor (:govern) and the
  ...
  operating history is advanced by MANY operations
  ...
  Governor is clean -- see `oleaginousops.governor/always-escalate-ops`.

  NOTE: langgraph-clj StateGraph integration is deferred (mirrors
  `berrynutops.operation`, cloud-itonami-isic-0125). This stub version
  ...
  (:require [oleaginousops.advisor :as advisor]
...
(defn run-operation
  "Run one oleaginous-fruit plantation operation through the advisor ->
...
(defn build
  ...
  run-operation for testing. opts:
    :advisor -- an `oleaginousops.advisor/Advisor` (default: mock-advisor)"
  [store & [opts]]
